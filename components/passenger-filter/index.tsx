            ...
            <Container>
                    ...
                    items?.map((item,index) => <nav key={get(item,'uuid')}
                        ...
                        <div className="flex justify-between flex-wrap flex-grow lg:gap-4 xl:gap-10">
                            <div className="order-1">
                                ...
                                <p className="lg:text-xs xl:text-sm font-medium">{dayjs(get(item, 'startDate')).format("D-MMM")}, {get(item, 'fromCity')}</p>
                            </div>
                            <div
                                className="order-3 lg:order-2 w-full lg:w-auto flex justify-between mt-5 lg:mt-0 border-b-2 border-primary-red py-2.5 flex-grow">
              <span>
                <svg
                    width="20"
                    height="20"
                ...
                >
                  <g clip-path="url(#clip0_577_8904)">
                    <path
                  ...
                  <defs>
                    <clipPath id="clip0_577_8904">
                      <rect width="20" height="20" fill="white"/>
                                ...
                                <h6 className="text-base font-semibold">{getTimeDifference(`${get(item, 'startDate')} ${get(item, 'starTime')}`, `${get(item, 'arrDate')} ${get(item, 'arrTime')}`)}</h6>
                                <span>
                <svg
                    width="20"
                    height="20"
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_577_8907)">
                    <path
                    ...
                    <clipPath id="clip0_577_8907">
                      <rect width="20" height="20" fill="white"/>
                            ...
                            <div className="order-2">
                                <h5 className="text-lg xl:text-xl font-semibold mb-2">{get(item, 'arrTime')}</h5>
                                <p className="lg:text-xs xl:text-sm font-medium">
                                    {dayjs(get(item, 'arrDate')).format("D-MMM")}, {get(item, 'toCity')}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col lg:flex-row justify-between flex-grow items-center">
                            <div>
                                <h5 className="text-lg xl:text-xl font-semibold mb-2">
                                    {get(item, 'family')} {get(item, 'name')}
                                </h5>
                                <p className="lg:text-xs xl:text-sm font-medium">
                                    {get(item, 'airlinesType')} — {get(item, 'ticketNumber')}
                                </p>
                            </div>
                            <button
              ...
              <span>
                <svg
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_577_5700)">
                    <path
